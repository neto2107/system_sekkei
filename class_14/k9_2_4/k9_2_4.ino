...
int rotate_time=2000,move_time=1000; //動く時間を指定　
...
void task() {
  switch (mode) {
    ...
    case FORWARD: //直進
      ...
      if(millis() -start_time >move_time){//1秒間直進したら回転する
        mode=ROTATE;
        start_time = millis();
      }
      break;
    case FORWARD2: 
      ...
      break;
    case ROTATE: //回転
      if (millis() - start_time < rotate_time) { //1秒回転
