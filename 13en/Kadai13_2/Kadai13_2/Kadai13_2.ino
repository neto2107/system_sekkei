...
void moveRobot(){
  ...
  if(mode == 1){//未発見状態だったら左回転
    ...
    motors.setLeftSpeed(-150);
    motors.setRightSpeed(150);
    timeDist = calSensor();//カップを見つけてから見失うまでの時間を算出
  ...
  if(mode ==3){
    ...
    if(sensingRobot(timeDist)==1){//回転しすぎた分、逆回転させる。
      mode = 4;
    }
  }else if(mode ==4){//探索が終了したとき
    motors.setLeftSpeed(100); //前進させる
    motors.setRightSpeed(100);
    if(dist>=30 || dist ==0){ //カップを見失ったら、リセットし、再び探索に戻る
      mode = 0; //リセット
    }
    if(dist<=4 && dist >0){ //カップが4cm以下に近づいたら、ロボットを停止させる。
      mode = 5;
    }
  }
  if(mode == 5){//キャッチしたら
    ...
    if(dist>=30 || dist ==0){ //カップを見失ったらリセット
      mode = 0; //リセット
    }
  }
...
int calSensor(){
  ...
  if((dist >= 30||dist ==0) && mode2 == 1){//カップを検出後、見失ったら
    mode2 = 0; //モードリセット
