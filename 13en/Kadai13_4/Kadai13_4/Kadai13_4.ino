...
int distance() {
  ...
  int avg = moving_average(dist);                       //移動平均を算出してavgに代入

  delay(60);                     //trigがHIGHになる間隔を60ms以上空ける（超音波センサの仕様）

  return avg; //distの移動平均を返す。
}
...
void moveRobot(){
  ...
  if(mode == 1){//未発見状態だったら左回転
    ...
    if(timeoutflag ==0){
      timePrev3 = timeNow;
      timeoutflag = 1;
    }else if(timeoutflag ==1&& timeNow-timePrev3 > 1000){
      mode =1;
      timeoutflag =0;
    }
...
void tansaku(){
  if(mode2 == 0){ //モードが0の時回転
    ...
    motors.setLeftSpeed(100);
    motors.setRightSpeed(100);
    if(timeNow-timePrev3 >=(int)random(1000,3000)){//1秒~3秒前進したら回転に戻る
      mode2 = 0;
      timePrev3 = timeNow;
    }
  }
